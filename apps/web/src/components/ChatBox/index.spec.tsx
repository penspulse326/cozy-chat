...
describe('ChatBox', () => {
  ...
  it('當有訊息時，應正確渲染訊息列表', () => {
    ...
    // MessageContent 元件會被渲染，可以透過其內容來驗證
    const messageElements = screen.getAllByRole('article'); // 假設 MessageContent 的根元素是 <article>
    // 為了讓這個測試更健壯，我們應該在 MessageContent 元件中加入 role="article"
    // 不過，目前我們可以先基於渲染的文字內容來判斷
    expect(messageElements.length).toBe(mockMessages.length);
  });

  it('應根據 userId 正確判斷是否為用戶本人訊息', () => {
    // 這個測試需要檢查傳遞給 MessageContent 的 isUser prop
    // 這通常需要對 MessageContent 進行 mock，或者在 MessageContent 內部渲染一些可供測試的標記
    ...
    // 為了達成這個測試，我們需要修改 MessageContent 來加上類似 data-testid="message-is-user-true" 的屬性
    // 這裡僅為示意，實際執行需要修改 MessageContent 元件
    expect(screen.getByText('嗨，你好！').closest('div[data-is-user="true"]'));
    expect(screen.getByText('你好啊！').closest('div[data-is-user="false"]'));
  });
});

// 為了讓上面的測試能跑，我們需要稍微修改 MessageContent/index.tsx
